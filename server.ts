...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'dist-video/video-handyman-tube/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(join(process.cwd(), 'dist-video/video-handyman-tube/browser'), 'index.html')
    )
    .toString();
    ...
    res.render(
      ...
      async (err, html) => {
        ...
        var seo: any = {
          ...
          image:
            'https://video.handyman.tube/assets/banner/HT-banner-default.png',
          site: 'https://video.handyman.tube/',
          url: 'https://video.handyman.tube' + params,
          keywords: 'Handyman Tube',
        };
        if (
          ...
          const talent = {
            name: post?.title || post?.albumname || 'Handyman.Tube Post',
